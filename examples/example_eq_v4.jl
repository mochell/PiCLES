...
fake_winds(x, t) = 0.01 + t * 0 + U10 * IfElse.ifelse.(    (x .> Lx/3.0) & (x .< Lx*2/3.0) , 1+ 0 .* x, 0 .* x)
...
particle_equations = PW.particle_equations(u, γ=γ)
...
particle_defaults = ParticleDefaults(log(FetchRelations.Eⱼ(1.0, DT)), 1e-1, 0.0)
...
wave_model = WaveGrowthModels1D.WaveGrowth1D(; grid=grid1d,
        ...
        periodic_boundary=false,
        boundary_type="wind_sea" #"wind_sea" 
        )
...
Revise.retry()
# %% run simulation
wave_simulation = Simulation(wave_model, Δt=20minutes, stop_time=12hours)
...
run!(wave_simulation, store=false, cash_store=true)
